...
// x 表示一个接口的类型，T 表示一个具体的类型（也可为接口类型），该断言表达式会返回 x 的值（也就是 value）和一个布尔值（也就是 ok），可根据该布尔值判断 x 是否为 T 类型
```
...
```go
文件对象,错误信息 := os.Create('FILENAME')

// 序列化为json
newencode
```

![image-20260719182618236](assets/image-20260719182618236.png)
...
上述代码执行，在代码所在文件夹的同级目录创建文件info.json，内容如下
...
func main() {
    filePtr, err := os.Open("/guide/info.json")
    if err != nil {
...
func main() {
	// 实例化
	info := Website{"Go语言教程", "http://c.biancheng.net/golang/", []string{"Go语言入门教程", "Golang入门教程"}}
	f, err := os.Create("./xmlF/info.xml")
	if err != nil {
		fmt.Println("文件创建失败", err.Error())
		return
	}
...
![image-20260719190440361](assets/image-20260719190440361.png)
...
func main() {
	// 打开xml文件
	file, err := os.Open("./xmlF/info.xml")
	if err != nil {
		fmt.Println("文件打开失败：%v", err)
		return
	}
	defer file.Close()
...
/**
解码成功
{Go语言教程 http://c.biancheng.net/golang/ [Go语言入门教程 Golang入门教程]}
 */
```
...
func main() {
	...
	D.Decode(&M)
	fmt.Println(M) // map[name:Go语言教程 website:http://c.biancheng.net/golang]
}
```
